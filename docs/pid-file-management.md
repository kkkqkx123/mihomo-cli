...
**基础目录**：
- 首选：`~/.mihomo-cli/`（用户主目录）
- 备选：`%TEMP%/mihomo-cli.pid`（无法获取用户目录时）
...
| 配置文件路径 | PID 文件路径 |
|-------------|-------------|
| (未指定) | `~/.mihomo-cli/mihomo.pid` |
| `config.yaml` | `~/.mihomo-cli/mihomo-config.pid` |
| `scripts/mihomo-config.yaml` | `~/.mihomo-cli/mihomo-mihomo-co.pid` |
| `E:\project\mihomo-go\test-config.yaml` | `~/.mihomo-cli/mihomo-test-con.pid` |
